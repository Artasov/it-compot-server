...
def handle_teachers_schedule_upload(request) -> dict:
    """
    ...
    if form.is_valid():
        try:
            schedule_dataframe = process_teachers_schedule_xlsx(form.cleaned_data['file'])
...
def process_teachers_schedule_xlsx(xlsx_file) -> DataFrame:
    """
    ...
    """
    teachers_parsed_schedule = parse_teachers_schedule_from_dj_mem(xlsx_file)
